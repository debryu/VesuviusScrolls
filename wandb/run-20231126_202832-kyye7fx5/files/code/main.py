...
def main():
    ...
    scheduler = get_scheduler(optimizer, multiplier=1, warmup_epoch=opts.warmup_epochs, cosine_epoch=opts.cosine_epochs)
    scaler = GradScaler()
    '''----------------------------------------------------------------------------------------------------'''
    ...
    for epoch in training_range:  
        losses, m_losses = train(model, train_dl, opts, epoch, optimizer, scaler, use_wandb=WANDB)
        ...
        if (epoch+1) % opts.interval_val == 0:
            losses, m_losses = evaluate(model, validation_dl, opts, epoch)
            final_loss = np.mean(losses)
            if WANDB:
                wandb.log({
